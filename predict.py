...
# ฟังก์ชันดึงข้อมูลบริษัท พร้อม Cache (ช่วยลดการโดนบล็อก)
@st.cache_data(ttl=24*3600)
def get_wiki_company_info(label_name):
    # ตั้งค่าภาษาไทย
    wikipedia.set_lang("th")
    
    # 1. แยกชื่อหุ้นออกจากวงเล็บ Ticker
    # ตัวอย่าง: "Apple (AAPL)" -> ได้ clean_name = "Apple"
    clean_name = label_name.split('(')[0].strip()
    
    # 2. สร้างรายการคำค้นหา (Search Candidates) เรียงตามลำดับความแม่นยำ
    search_candidates = [
        f"{clean_name} (บริษัท)",   # สูตร 1: เติม (บริษัท) ต่อท้าย (แม่นสุดใน Wiki ไทย)
        f"บริษัท {clean_name}",     # สูตร 2: เติม บริษัท ข้างหน้า
        f"{clean_name} Inc.",      # สูตร 3: ลองชื่ออินเตอร์
        clean_name                 # สูตร 4: ชื่อเพียวๆ (ทางเลือกสุดท้าย)
    ]
    
    # พิเศษ: แก้คำเฉพาะที่มักจะเพี้ยนบ่อยๆ (Hardcode fix)
    if "Amazon" in clean_name:
        search_candidates.insert(0, "อเมซอน.คอม") # Wiki ไทยใช้ชื่อนี้
    elif "Meta" in clean_name or "Facebook" in clean_name:
        search_candidates.insert(0, "เมตา แพลตฟอร์มส์")
    elif "Google" in clean_name:
        search_candidates.insert(0, "กูเกิล")
    elif "PTT" in clean_name or "ปตท" in clean_name:
        search_candidates.insert(0, "ปตท. (บริษัท)")

    # 3. วนลูปค้นหาทีละสูตร
    found_summary = None
    found_url = None
    ...
    for query in search_candidates:
        try:
            # ลองดึงข้อมูลดู
            page = wikipedia.page(query, auto_suggest=False) # auto_suggest=False คือห้ามเดามั่ว
            
            # เช็คความชัวร์: ถ้าในเนื้อหาไม่มีคำว่า "บริษัท", "Inc", "corp", "ธุรกิจ" แสดงว่าผิดตัวแน่ๆ
            content_snippet = page.content[:500]
            keywords = ["บริษัท", "Inc", "Corporation", "Holding", "จำกัด", "มหาชน", "ธุรกิจ", "ก่อตั้ง", "หุ้น"]
            
            if any(k in content_snippet for k in keywords):
                found_summary = page.summary
                found_url = page.url
                break # เจอแล้วหยุดลูปเลย
            
        except (wikipedia.exceptions.PageError, wikipedia.exceptions.DisambiguationError):
            continue # ถ้าสูตรนี้ Error ให้ข้ามไปลองสูตรถัดไป
        except Exception:
            continue

    # 4. ตัดบทความให้สั้นลง (เอาแค่ 500 ตัวอักษรพอ)
    if found_summary:
        if len(found_summary) > 600:
            found_summary = found_summary[:600] + "..."
...
if wiki_summary:
    ...
    st.markdown("*(ระบบพยายามค้นหาแล้วแต่ไม่พบข้อมูลที่ตรงกับบริษัทนี้โดยเฉพาะ)*")
# ----------------------------------------------------------------------
