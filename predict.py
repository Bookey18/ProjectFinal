...
try:
    ...
    # แปลเป็นภาษาไทย (ใช้ st.session_state เพื่อ cache การแปล)
    translator = Translator()
    
    # ใช้ try/except เพื่อจัดการปัญหาการเชื่อมต่อหรือการแปล
    try:
        translation = translator.translate(summary_en, src='en', dest='th')
        summary_th = translation.text
    except Exception:
        summary_th = "ไม่สามารถแปลสรุปธุรกิจได้ กรุณาลองอีกครั้งในภายหลัง"
    ...
    st.warning(f"⚠️ ไม่สามารถโหลดหรือแสดงข้อมูลบริษัทของ {selected_label} ได้")

# ----------------------------------------------------------------------
# 4. กราฟราคาย้อนหลัง พร้อมตัวเลือกเดือน/ปี

st.markdown("---")
st.subheader('📈 กราฟราคาย้อนหลัง (Price Chart)')
...
data_load_state = st.info('📥 กำลังดาวน์โหลดข้อมูลราคาย้อนหลัง...')
...
def plot_candlestick_chart(df, label):
    ...
    fig.update_layout(
        title=f'กราฟแท่งเทียนราคาย้อนหลังของ {label} (ตั้งแต่วันที่ {start_date.strftime("%Y-%m-%d")})',
        xaxis_title='วันที่',
        yaxis_title='ราคา',
...
def plot_line_chart(df, label):
    ...
    fig.update_layout(
        title=f'กราฟเส้นราคาปิดย้อนหลังของ {label} (ตั้งแต่วันที่ {start_date.strftime("%Y-%m-%d")})',
        xaxis_title='วันที่',
        yaxis_title='ราคา',
        xaxis_rangeslider_visible=True, 
        template='plotly_white',
...
if not data.empty:
    ...
    st.subheader('ตารางข้อมูลราคาย้อนหลัง (Raw Data)')
    st.dataframe(data.tail(20).set_index('Date').style.format('{:.2f}'), use_container_width=True)

    # ----------------------------------------------------------------------
    # 6. เลือกจำนวนปีที่ต้องการทำนาย (พร้อมหัวข้อเด่นชัด)

    st.markdown("---")
    st.markdown("## 🔮 ส่วนการทำนายราคาในอนาคต")
    st.markdown("---")
    ...
    with col_pred_select:
        # ปีที่ต้องการทำนาย
        n_years = st.slider('เลือกจำนวนปีที่ต้องการทำนาย (สูงสุด 4 ปี):', 1, 4)
        period = n_years * 365
    
    with col_pred_info:
        st.info(f"ระบบจะใช้ข้อมูลตั้งแต่ **ปี {data['Date'].min().year}** ถึง **ปัจจุบัน** เพื่อสร้างแบบจำลองและคาดการณ์ราคาไปอีก **{n_years} ปี** ข้างหน้า")
    ...
    try:
        ...
        # แสดงผลการทำนาย
        st.subheader(f'ผลลัพธ์การคาดการณ์ราคาของ {selected_label}')
        
        # ตารางผลการทำนาย
        st.markdown("#### ตารางผลการคาดการณ์ (10 แถวล่าสุด)")
        forecast_display = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(10)
        forecast_display.columns = ['วันที่', 'ราคาคาดการณ์ (yhat)', 'ช่วงต่ำสุด (Lower)', 'ช่วงสูงสุด (Upper)']
        st.dataframe(forecast_display.set_index('วันที่').style.format({'ราคาคาดการณ์ (yhat)': '{:.2f}', 'ช่วงต่ำสุด (Lower)': '{:.2f}', 'ช่วงสูงสุด (Upper)': '{:.2f}'}), use_container_width=True)

        # กราฟคาดการณ์
        st.markdown(f'#### กราฟคาดการณ์ {n_years} ปีข้างหน้า')
