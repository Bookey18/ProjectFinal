...
@st.cache_data(ttl=24*3600)  # เก็บข้อมูลไว้ 24 ชม. ไม่ต้องโหลดใหม่
...
# ฟังก์ชันแปลภาษา
def safe_translate_deep(text):
    from deep_translator import GoogleTranslator
    try:
        translator = GoogleTranslator(source='auto', target='th')
        return translator.translate(text)
    except Exception as e:
        return "⚠️ แปลภาษาไม่ได้ชั่วคราว"

# เริ่มกระบวนการแสดงผล
info = get_stock_info(selected_stock)
    
if info:
    # ดึงตัวแปรต่างๆ (ใช้ .get เพื่อป้องกัน Error ถ้าไม่มีข้อมูล)
    long_name = info.get('longName', selected_label)
    ...
    if selected_stock in st.session_state.translated_cache:
        ...
        if summary_en:
            summary_th = safe_translate_deep(summary_en)
        else:
            summary_th = "ไม่มีข้อมูลรายละเอียดบริษัท"
        st.session_state.translated_cache[selected_stock] = summary_th

    # ---------------- แสดงผล (ย้ายมาไว้ใน if info: เพื่อความปลอดภัย) ----------------
    ...
    with col_info2:
        ...
        if website != '#':
            ...
            st.markdown("**เว็บไซต์:** ไม่มีข้อมูล")

    st.markdown("### 📘 สรุปธุรกิจ (ภาษาไทย)")
    st.info(summary_th) # ใช้ st.info ให้ดูสวยงามขึ้น
    
    with st.expander("ดูต้นฉบับภาษาอังกฤษ (Original English Text)"):
        st.write(summary_en)

else:
    # กรณีดึงข้อมูลไม่ได้ ให้แสดงข้อความเตือนแทนการ Error
    st.warning(f"⚠️ ไม่สามารถดึงข้อมูลบริษัท {selected_label} ได้ในขณะนี้ (อาจเกิดจาก Yahoo Finance จำกัดการเข้าถึง) แต่กราฟราคายังทำงานได้ปกติ")
# ----------------------------------------------------------------------
