...
try:
    ...
    if selected_stock in st.session_state.translated_cache:
        ...
        # ฟังก์ชันแปลใหม่ ใช้ deep_translator
        def safe_translate_deep(text):
            try:
                # deep-translator รองรับข้อความยาวได้ดีกว่า แต่ถ้าจะให้ชัวร์ก็ตัดแบ่งได้
                # ในที่นี้ deep-translator จัดการ auto split ได้ดีระดับหนึ่ง
                translator = GoogleTranslator(source='auto', target='th')
                return translator.translate(text)
            except Exception as e:
                return f"⚠️ แปลภาษาไม่ได้: {e}"

        if summary_en and summary_en != 'ไม่มีข้อมูล':
    ...
    # เพิ่มส่วนแสดงภาษาอังกฤษเผื่อไว้กรณีแปลไม่ได้จริงๆ
